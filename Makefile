...
SOURCE_DIRS	:= source := include := source/irq_handling := source/audio := data/audio/samples_processed := data/audio/assets_processed \
			:= data/audio/assets_priorities := source/scenes := data/graphics
SOURCE_EXTS	:= c := s := S
...
ALLFLAGS	:= -Wall -Wextra -g3 -gdwarf-4 -O2 \
		...
		-D_DEFAULT_SOURCE -mthumb-interwork \
